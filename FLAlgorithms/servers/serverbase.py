...
class Server:
    def __init__(self, device, dataset,algorithm, model, batch_size, learning_rate, L_k,
    ...
    def test(self):
        ...
        mean_accurancy = []
        for c in self.users:
            ct, ns, ma = c.test() # ma = ct(总ACC) / ns(数量)
    ...
    def evaluate(self):
        stats = self.test()  # [ids, num_samples, tot_correct, mean_accurancy]
