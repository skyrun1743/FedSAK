...
def read_user_data(index,data,dataset):
    ...
    if(dataset == "mnist"):
        ...
        X_train, y_train, X_test, y_test = train_data['x'], train_data['y'], test_data['x'], test_data['y']
        X_train = torch.Tensor(np.array(X_train)).view(-1, NUM_CHANNELS_CIFAR, IMAGE_SIZE_CIFAR, IMAGE_SIZE_CIFAR).type(torch.float32).contiguous()
        y_train = torch.Tensor(np.array(y_train)).type(torch.int64).contiguous()
        X_test = torch.Tensor(np.array(X_test)).view(-1, NUM_CHANNELS_CIFAR, IMAGE_SIZE_CIFAR, IMAGE_SIZE_CIFAR).type(torch.float32).contiguous()
        y_test = torch.Tensor(np.array(y_test)).type(torch.int64).contiguous()
    elif (dataset == 'pacs'):
        X_train = torch.Tensor(np.array(X_train)).type(torch.float32).contiguous()
        y_train = torch.Tensor(np.array(y_train)).type(torch.int64).contiguous()
        X_test = torch.Tensor(np.array(X_test)).type(torch.float32).contiguous()
        y_test = torch.Tensor(np.array(y_test)).type(torch.int64).contiguous()
    else:
